...
const ShiftPerspectiveInputSchema = z.object({
  ...
  characterName: z
    .string()
    .describe('The name of the character whose perspective we should adopt.'),
  role: z
    .enum(['Protagonist', 'Antagonist'])
    .describe('The new role to cast this character in for the summary.'),
  format: z
    .enum(['summary', 'diaryEntry', 'letter', 'policeStatement'])
    .describe('The format of the rewritten text.'),
});
...
export async function shiftPerspective(
  input: ShiftPerspectiveInput
): Promise<ShiftPerspectiveOutput> {
