...
const ApplyNarratorBiasInputSchema = z.object({
  ...
  bias: z
    .object({
      startBias: NarratorBiasEnum.describe(
        "The narrator's bias at the beginning of the story."
      ),
      endBias: NarratorBiasEnum.describe(
        "The narrator's bias at the end of the story."
      ),
    })
    .describe('The evolving bias to apply to the narrator.'),
});
export type ApplyNarratorBiasInput = z.infer<
  typeof ApplyNarratorBiasInputSchema
>;

const ApplyNarratorBiasOutputSchema = z.object({
  biasedStoryText: z
    .string()
    .describe('The rewritten story text with the applied narrator bias.'),
});
export type ApplyNarratorBiasOutput = z.infer<
  typeof ApplyNarratorBiasOutputSchema
>;

export async function applyNarratorBias(
  input: ApplyNarratorBiasInput
): Promise<ApplyNarratorBiasOutput> {
